...
- **工作区文件变动监听**：新增可选的「自动监听文件变动」设置，默认关闭。开启后，Web 端通过前台轮询与焦点恢复扫描，桌面端通过 Tauri 原生递归监听，外部新增、修改或删除 Markdown 后自动更新列表、标签、搜索索引和详情。
- **ID 完整性修复**：启动扫描和文件监听会自动修复缺失、非法或重复的 Prompt ID。复制已有 Markdown 时，原文件保留 ID，新文件生成唯一 ID；修复过程只修改 frontmatter 中的 ID，并保留其他字段与正文。
- **元数据修复提醒**：启动或检测到新增 Markdown 缺少 PromptClip 元数据时，新增应用内提醒，可选择自动修复、查看详情或本次忽略。
